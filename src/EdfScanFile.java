...
public class EdfScanFile {
    private final String filePath;
    private ArrayList<EdfSourceDiskRequestData> items;

    public EdfScanFile(String filePath) {
    ...
    private void readLine(int lineNo, String line) {
        if (lineNo == 0 && line.startsWith("request_no"))
            return;

        try {
            var item = new EdfSourceDiskRequestData(line);
            items.add(item);
        }
        catch (Exception ex) {
    ...
    public List<EdfSourceDiskRequestData> getItems() {
        return items;
    }
}